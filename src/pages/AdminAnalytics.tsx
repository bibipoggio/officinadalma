...
function useActiveUsersToday() {
  return useQuery({
    queryKey: ["active-users-today"],
    queryFn: async (): Promise<ActiveUser[]> => {
      const { data, error } = await supabase.rpc("get_active_users_today" as any);
      ...
      <div className="space-y-6 max-w-6xl mx-auto">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            />
            <AnalyticsMetricCard
              title="Reproduções Enraizamento"
              value={enhancedAnalytics?.enraizamento_plays || 0}
              subtitle={`${enhancedAnalytics?.enraizamento_unique_users || 0} usuários`}
              icon={<Volume2 className="w-5 h-5" />}
            />
            <AnalyticsMetricCard
              title="Reproduções Meditação"
              value={enhancedAnalytics?.meditation_plays || 0}
              subtitle={`${enhancedAnalytics?.meditation_unique_users || 0} usuários`}
              icon={<Headphones className="w-5 h-5" />}
            />
            <AnalyticsMetricCard
              title="Check-ins"
              value={enhancedAnalytics?.checkins_today || 0}
              subtitle={`${enhancedAnalytics?.checkins_unique_users || 0} usuários`}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2">
              ...
              Pessoas Ativas Hoje ({activeUsers?.length || 0})
            </CardTitle>
          </CardHeader>
          <CardContent>
            {activeUsersLoading ? (
              ...
              <p className="text-sm text-muted-foreground">Nenhum usuário ativo hoje</p>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {activeUsers!.map((u) => (
                  <div key={u.user_id} className="flex items-center gap-3 p-2 rounded-lg bg-muted/50">
